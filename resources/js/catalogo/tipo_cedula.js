...
document.querySelector('.titulo').textContent = 'Tipo de cédula'
...
const datos = [
    ['tipo_cedula','Tipo de cédula','text']
]
...
btn_guardar.addEventListener('click', e => {
    ...
    validar_datos('tipos_cedulas','POST')
...
btn_editar.addEventListener('click', e => {
    ...
    validar_datos(`tipos_cedulas/update/${sessionStorage.getItem('id')}`,'PATCH')
...
window.addEventListener('click', e => {
    ...
    if (e.target.classList.contains('btn-ico-editar')) {
        ...
        for (let i = 0; i < datos.length; i++) {
            input(datos[i][0],datos[i][1],datos[i][2],true,e.target.getAttribute('data-id'),'tipos_cedulas',datos[i][3],datos[i][4])
        }
    ...
    if (e.target.classList.contains('btn-delete')) {
        ...
        fun_fetch(`tipos_cedulas/delete/${id}`,'','DELETE')
        .then(respuesta => {
            if (respuesta.ok) {
                Swal.fire({
    ...
    if (e.target.classList.contains('btn-ico-view')){
        ...
        cambiar_a_view()
        fun_fetch(`tipos_cedulas_x_id/${e.target.getAttribute('data-id')}`,'','GET')
        .then(res => res.json())
        .then(data => {
            for (let i = 0; i < datos.length; i++) {
                if (datos[i][2] == 'select') {
