...
document.querySelector('.titulo').textContent = 'Contratistas / Equipos'
...
const encabezado = [
    ...
    'Número de serie',
    {
        ...
        formatter: (cell, row) => gridjs.html(`
            <i class="fa-regular fa-eye btn-ico-view btn-acciones" data-id="${row.cells[4].data}"></i>
            <i class="fa-solid fa-pen-to-square btn-ico-editar btn-acciones" data-id="${row.cells[4].data}"></i>
            <i class="fa-regular fa-square-minus btn-delete btn-acciones" data-id="${row.cells[4].data}"></i>
        `)
    }
]
...
const data = {
        ...
        .map(item => [
            ...
            item.tipos_equipos.tipo_equipo,
            item.equipo,
            item.numero_serie,
            item.id
        ]
...
const datos = [
    ...
    ['id_tipo_equipo','Tipo de equipo','select','tipos_equipos','tipo_equipo'],
    ['equipo','Equipo','text'],
...
btn_editar.addEventListener('click', e => {
    ...
    validar_datos(`equipos/update/${sessionStorage.getItem('id')}`,'PATCH')
...
window.addEventListener('click', e => {
    ...
    if (e.target.classList.contains('btn-ico-editar')) {
        ...
        for (let i = 0; i < datos.length; i++) {
            input(datos[i][0],datos[i][1],datos[i][2],true,e.target.getAttribute('data-id'),'equipos',datos[i][3],datos[i][4])
        }
    ...
    if (e.target.classList.contains('btn-delete')) {
        ...
        fun_fetch(`equipos/delete/${id}`,'','DELETE')
        .then(respuesta => {
            if (respuesta.ok) {
                Swal.fire({
    ...
    if (e.target.classList.contains('btn-ico-view')){
        ...
        cambiar_a_view()
        fun_fetch(`equipos_x_id/${e.target.getAttribute('data-id')}`,'','GET')
        .then(res => res.json())
        .then(data => {
            for (let i = 0; i < datos.length; i++) {
                if (datos[i][2] == 'select') {
